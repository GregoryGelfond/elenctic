...
Checks are pure over a ``SolveOutcome``; only ``solvers.py`` ever runs a solve. The ``Symbol`` this
module imports is clingo's value vocabulary, not a solver — a check reads and builds symbols (and
