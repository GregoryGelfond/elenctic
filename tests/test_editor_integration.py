...
# What the editor draws for each of the three verdicts. Only one of them is a failure: `undecided`
# is an absence of knowledge rather than a wrong answer, so a consumer that drew it as an error
# would report a program as broken on the strength of a search that ran out of time.
...
def _diagnostics(case: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        {
            "uri": case["source"],
            # An editor counts lines from zero; a contract line is written from one.
            "range": {"start": {"line": check["line"] - 1}, "end": {"line": check["line"] - 1}},
...
def test_a_single_file_run_yields_a_placeable_diagnostic_per_failing_check(
    ...
    assert diagnostics[0]["uri"].endswith("drinks.lp")
