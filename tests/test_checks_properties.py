...
These pin the structural guarantees the example tests only sample: consequence-soundness
(an incomplete solve is *always* UNDECIDED, never FAIL/raise), the variadic optimal
aggregation (⋂/⋃ over the whole class equals the pairwise fold, singleton included), and
containment soundness (PASS iff ⊆). Verification-engineer Pass-A recommendation.
"""
...
@given(_atom_sets)
def test_every_check_is_undecided_on_an_incomplete_solve(litset: frozenset[Symbol]) -> None:
    incomplete = SolveResult(completed=False)
    for check in _every_check(litset):
        assert check(incomplete).verdict is Verdict.UNDECIDED  # §7a — never FAIL, never raises
...
def test_optimal_aggregation_equals_the_pairwise_fold(family: list[frozenset[Symbol]]) -> None:
    # The check's variadic ⋂/⋃ over Opt(P) must equal the pairwise fold for every family
    # size — singleton included (the ``family[0].intersection(*family[1:])`` edge).
    result = SolveResult(True, optimal_observables=tuple(Observable(s) for s in family))
...
def test_cautious_passes_iff_subset(
    ...
    result = SolveResult(True, intersection=aggregate)
