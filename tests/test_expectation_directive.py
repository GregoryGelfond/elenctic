"""@elenctic solver: the declared frame, well-formedness, and membership."""
...
def test_absent_directive_leaves_solver_undeclared() -> None:
    # None = undeclared; discovery defaults it to "clingo" (the stated default).
    contract = parse_contract("% @expect sat\n% @model { a }\na.\n", source="case.lp")
    assert contract.solver is None


def test_unknown_solver_is_a_contract_error_with_provenance() -> None:
...
def test_repeated_solver_directive_is_a_contract_error() -> None:
...
def test_empty_solver_payload_is_a_contract_error() -> None:
    with pytest.raises(ContractError, match=r"@elenctic solver needs a solver name"):
        parse_contract("% @elenctic solver\n% @expect sat\n", source="case.lp")


def test_unknown_elenctic_subdirective_is_a_contract_error() -> None:
...
def test_behavioral_parse_is_unchanged_by_the_directive() -> None:
    # @elenctic does NOT thread through the behavioral builder: it is routed away.
...
def test_elenctic_is_never_reported_as_an_unknown_contract_tag() -> None:
    # Regression: @elenctic is routed away before the behavioral closed-vocab check,
