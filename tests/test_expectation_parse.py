...
def cell[T](claim: Claimed[T] | None) -> T:
    """What an occupied single-valued cell holds. These tests are about which cell a tag lands in
    and what it parses to; the line each claim also carries is ``test_contract_lines``'s subject."""
    assert claim is not None, "the cell should be occupied"
    return claim.value


def cells[T](claims: tuple[Claimed[T], ...]) -> list[T]:
    """What a repeatable tag's claims hold, in surface order."""
...
def test_bare_witness_carries_an_empty_assignment() -> None:
    ...
    exp = parse("% @expect sat\n% @model { a, b }\n")
    assert isinstance(exp, Sat)
    assert cell(exp.model) == WitnessClaim(shown=L("a", "b"))
    assert cell(exp.model).assign == frozenset()
...
def test_parse_sat_with_cost_and_optimal_witness() -> None:
    ...
    assert isinstance(exp, Sat)
    assert cell(exp.cost) == (4, 2)
    assert cell(exp.optimal_model) == WL(
        "included(s,a,2,1)", "included(a,t,2,1)", "start(s)", "end(t)"
    )
...
def test_cost_single_component() -> None:
    exp = parse("% @expect sat\n% @cost { 8 }\n")
    assert isinstance(exp, Sat)
    assert cell(exp.cost) == (8,)


def test_cautious_accumulates_at_base() -> None:
    exp = parse("% @expect sat\n% @cautious { start(s) }\n% @cautious { end(t) }\n")
    assert isinstance(exp, Sat)
    # two lines, two claims, in surface order — the cell no longer unions them
    assert cells(exp.cautious) == [L("start(s)"), L("end(t)")]
    assert exp.cautious_optimal == ()


def test_cautious_optimal_is_a_distinct_cell_from_cautious() -> None:
    exp = parse("% @expect sat\n% @cautious { p }\n% @cautious optimal { q }\n")
    assert isinstance(exp, Sat)
    assert cells(exp.cautious) == [L("p")]
    assert cells(exp.cautious_optimal) == [L("q")]


def test_brave_accumulates_and_brave_optimal_is_distinct() -> None:
    exp = parse("% @expect sat\n% @brave { a }\n% @brave { b }\n% @brave optimal { c }\n")
    assert isinstance(exp, Sat)
    assert cells(exp.brave) == [L("a"), L("b")]
    assert cells(exp.brave_optimal) == [L("c")]


def test_count_base_axis() -> None:
    exp = parse("% @expect sat\n% @count 3\n% @count optimal 2\n")
    assert isinstance(exp, Sat)
    assert cell(exp.count) == 3
    assert cell(exp.count_optimal) == 2


def test_parse_assign_bindings() -> None:
    exp = parse("% @expect sat\n% @assign { digit(s)=9, digit(e)=5 }\n")
    assert isinstance(exp, Sat)
    assert cell(exp.assign) == frozenset({(parse_term("digit(s)"), 9), (parse_term("digit(e)"), 5)})


def test_assign_accepts_a_negative_value() -> None:
    exp = parse("% @expect sat\n% @assign { offset(x)=-3 }\n")
    assert isinstance(exp, Sat)
    assert cell(exp.assign) == frozenset({(parse_term("offset(x)"), -3)})


def test_model_optimal_populates_the_optimal_cell() -> None:
    exp = parse("% @expect sat\n% @model optimal { a, b }\n")
    assert isinstance(exp, Sat)
    assert cell(exp.optimal_model) == WL("a", "b")
    assert exp.model is None
...
def test_optimal_is_sugar_for_model_optimal_and_coexists_with_model() -> None:
    ...
    assert isinstance(exp, Sat)
    assert cell(exp.model) == WL("a")
    assert cell(exp.optimal_model) == WL("b")


def test_litset_accepts_strong_negation_literal() -> None:
    exp = parse("% @expect sat\n% @cautious { -reachable(x) }\n")
    assert isinstance(exp, Sat)
    assert cells(exp.cautious) == [L("-reachable(x)")]


def test_litset_is_paren_aware() -> None:
    exp = parse("% @expect sat\n% @model { included(s,a,2,1), start(s) }\n")
    assert isinstance(exp, Sat)
    assert cell(exp.model) == WL("included(s,a,2,1)", "start(s)")


def test_queries_are_collected_in_order() -> None:
    exp = parse("% @expect sat\n% @query yes { start(s), end(t) }\n% @query no { reachable(x) }\n")
    assert isinstance(exp, Sat)
    parsed = cells(exp.queries)
    assert len(parsed) == 2
    assert all(isinstance(q, GroundQuery) for q in parsed)
    assert parsed[0].answer is Answer.yes
    assert parsed[1].answer is Answer.no
...
def test_continuation_joins_an_unclosed_litset() -> None:
    exp = parse("% @expect sat\n% @model { assign(s,9),\n%          assign(e,5) }\n")
    assert isinstance(exp, Sat)
    assert cell(exp.model) == WL("assign(s,9)", "assign(e,5)")
...
def test_prose_comment_between_tags_is_ignored() -> None:
    exp = parse("% @expect sat\n% @cautious { a }\n% just a stray remark\n% @brave { b }\n")
    assert isinstance(exp, Sat)
    assert cells(exp.cautious) == [L("a")]
    assert cells(exp.brave) == [L("b")]
...
def test_continuation_spans_three_lines() -> None:
    exp = parse("% @expect sat\n% @model { a,\n%          b,\n%          c }\n")
    assert isinstance(exp, Sat)
    assert cell(exp.model) == WL("a", "b", "c")


def test_cautious_optimal_accumulates() -> None:
    exp = parse("% @expect sat\n% @cautious optimal { a }\n% @cautious optimal { b }\n")
    assert isinstance(exp, Sat)
    assert cells(exp.cautious_optimal) == [L("a"), L("b")]
...
def test_assign_term_with_internal_comma_is_one_binding() -> None:
    ...
    exp = parse("% @expect sat\n% @assign { f(a,b)=1 }\n")
    assert isinstance(exp, Sat)
    assert cell(exp.assign) == frozenset({(parse_term("f(a,b)"), 1)})
...
def test_assign_optimal_is_a_distinct_cell_from_assign() -> None:
    exp = parse("% @expect sat\n% @assign { v=1 }\n% @assign optimal { w=2 }\n")
    assert isinstance(exp, Sat)
    assert cell(exp.assign) == frozenset({(parse_term("v"), 1)})
    assert cell(exp.assign_optimal) == frozenset({(parse_term("w"), 2)})


def test_where_witness_single_line() -> None:
    exp = parse("% @expect sat\n% @model { a } where { v=1 }\n")
    assert isinstance(exp, Sat)
    assert cell(exp.model) == WitnessClaim(shown=L("a"), assign=frozenset({(parse_term("v"), 1)}))
...
def test_where_witness_on_the_optimal_base() -> None:
    ...
    assert cell(exp.optimal_model) == WitnessClaim(shown=L("a"), assign=binding)
