...
def test_check_declares_what_it_reads_statically() -> None:
    ...
    assert cost_is((1,), line=1).reads == frozenset({Field.OPTIMUM})
    assert expect_unsat(line=1).reads == frozenset({Field.WITNESS})
...
def test_cost_compares_the_vector_by_value() -> None:
    assert (
        cost_is((4, 2), line=1)(decided(ConsistentOptimum(Optimum((4, 2))))).verdict is Verdict.PASS
    )
    missed = cost_is((4, 2), line=1)(decided(ConsistentOptimum(Optimum((4, 3)))))
    assert missed.verdict is Verdict.FAIL
    assert "4" in missed.message
    unsat = cost_is((4, 2), line=1)(decided(Inconsistent()))
...
def test_assign_is_existential_over_observables() -> None:
    ...
    empty = assign_contains(target, line=1)(decided(Inconsistent()))
    assert empty.verdict is Verdict.FAIL  # AS(P) = ∅ arm
...
def test_assign_finds_a_match_among_multiple_observables() -> None:
    ...
    assert (
        assign_contains(target, line=1)(decided(result)).verdict is Verdict.PASS
...
def test_where_witness_couples_shown_and_assignment_on_one_model() -> None:
    ...
    assert has_model(claim, line=1)(decided(coupled)).verdict is Verdict.PASS
    assert has_model(claim, line=1)(decided(split)).verdict is Verdict.FAIL
